...
import type { ConvertFormState, ConvertModalHint, ImageConvertOptions, MediaInfo, OutputSubfolderSettings, VideoConvertOptions } from './types.js';

const LAST_USED_SETTINGS_KEY = 'lastUsedConvertSettings';
const DEFAULT_OUTPUT_SUBFOLDER_NAME = 'converted';

const VIDEO_PRESET_IDS = ['discordGif', 'webGif', 'archiveMp4', 'socialShare', 'mp4Copy'] as const;
const IMAGE_PRESET_IDS = ['webOptimized'] as const;
const UNIVERSAL_PRESET_IDS = ['lastUsed', 'custom'] as const;

type VideoPresetId = typeof VIDEO_PRESET_IDS[number];
type ImagePresetId = typeof IMAGE_PRESET_IDS[number];
type UniversalPresetId = typeof UNIVERSAL_PRESET_IDS[number];
type PresetId = VideoPresetId | ImagePresetId | UniversalPresetId;

export const BOOLEAN_FORM_FIELDS = new Set<keyof ConvertFormState>([
...
export function getAvailablePresetIds(hasVideos: boolean, hasImages: boolean) {
  const presetIds: string[] = [...UNIVERSAL_PRESET_IDS];
...
export function isPresetAvailable(presetId: string, hasVideos: boolean, hasImages: boolean) {
  return getAvailablePresetIds(hasVideos, hasImages).includes(presetId);
}

export function getPresetOptions(hasVideos: boolean, hasImages: boolean) {
  const options = [
  ...
  if (hasVideos) {
    options.push(
      { value: 'discordGif', label: t('presetDiscordGif') },
      { value: 'webGif', label: t('presetWebGif') },
      { value: 'archiveMp4', label: t('presetArchiveMp4') },
      { value: 'socialShare', label: t('presetSocialShare') },
      { value: 'mp4Copy', label: t('presetMp4Copy') },
    );
  }

  if (hasImages) {
    options.push(
      { value: 'webOptimized', label: t('presetWebOptimized') },
    );
  }
...
export function sanitizeFormStateForSelection(
  formState: ConvertFormState,
  hasVideos: boolean,
  hasImages: boolean,
): ConvertFormState {
...
export function createDefaultFormState(): ConvertFormState {
  return {
    preset: 'lastUsed',
    videoFormat: 'mp4',
    videoCodecMode: 'auto',
...
export function sanitizeOutputSubfolderName(name: string) {
...
export async function getOutputSubfolderSettings(): Promise<OutputSubfolderSettings> {
  try {
    const useSubfolder = await sigma.settings.get<boolean>('defaultOutputSubfolder');
    const configuredName = await sigma.settings.get<string>('outputSubfolderName');
...
export function applyModalHint(
  formState: ConvertFormState,
  hint: ConvertModalHint | undefined,
  hasVideos: boolean,
  hasImages: boolean,
): ConvertFormState {
  if (!hint) {
...
export function applyPreset(presetId: string, formState: ConvertFormState): ConvertFormState {
  const next = { ...formState, preset: presetId };

  switch (presetId as PresetId) {
    case 'discordGif':
...
function migrateStoredFormState(stored: Record<string, unknown>, defaultState: ConvertFormState): ConvertFormState {
  const legacyScaleFields = migrateLegacyScaleFields(stored);
  const merged = { ...defaultState, ...stored, ...legacyScaleFields } as ConvertFormState;
  return merged;
}

export async function loadLastUsedFormState(defaultState: ConvertFormState): Promise<ConvertFormState> {
  try {
    const rememberLastUsed = await sigma.settings.get<boolean>('rememberLastUsed');
    if (rememberLastUsed === false) {
      return defaultState;
    }

    const stored = await sigma.storage.get<Record<string, unknown>>(LAST_USED_SETTINGS_KEY);
    if (!stored) {
      return defaultState;
    }
...
export async function saveLastUsedFormState(formState: ConvertFormState) {
  try {
    const rememberLastUsed = await sigma.settings.get<boolean>('rememberLastUsed');
    if (rememberLastUsed === false) {
      return;
...
export async function applyExtensionDefaults(
  formState: ConvertFormState,
  hasVideos: boolean,
  hasImages: boolean,
  ...
  try {
    const defaultPreset = await sigma.settings.get<string>('defaultPreset');
...
function isConvertFormField(elementId: string): elementId is keyof ConvertFormState {
  return elementId in createDefaultFormState();
}

export function mergeFormState(current: ConvertFormState, elementId: string, value: unknown): ConvertFormState {
  if (elementId === 'preset') {
...
function resolveSelectOrCustom(selectValue: string, customValue: string) {
...
export function parseVideoConvertOptions(
  formState: ConvertFormState,
  sourceMediaInfo: MediaInfo | null,
): VideoConvertOptions {
  ...
  return {
    ...
    videoFramerate: resolveFramerate(
      formState.framerateSelect,
      formState.framerateCustom,
      sourceMediaInfo?.frameRate ?? null,
      isGif,
    ),
...
export function parseImageConvertOptions(formState: ConvertFormState): ImageConvertOptions {
  return {
...
export function validateConvertFormState(
  formState: ConvertFormState,
  hasVideos: boolean,
  hasImages: boolean,
): string | null {
  ...
  if (hasVideos) {
    const scaleError = validateScaleFields(
      formState.videoScaleMode,
      formState.videoScaleWidth,
      formState.videoScaleHeight,
      formState.videoScalePercent,
    );
    if (scaleError) {
      return scaleError;
    }
  ...
  if (hasImages) {
    const scaleError = validateScaleFields(
      formState.imageScaleMode,
      formState.imageScaleWidth,
      formState.imageScaleHeight,
      formState.imageScalePercent,
    );
    if (scaleError) {
      return scaleError;
    }
...
export function buildVideoParamSuffix(options: VideoConvertOptions) {
  const parts: string[] = [];
...
export function buildImageParamSuffix(options: ImageConvertOptions) {
  const parts: string[] = [];
...
export function resolveVideoFramerateForFile(
  ...
  return {
    ...options,
    videoFramerate: resolveFramerate(
      options.framerateSelect,
      options.framerateCustom,
      sourceFrameRate,
      isGif,
    ),
  };
...
export function formStateFromRecord(values: Record<string, unknown>, base: ConvertFormState): ConvertFormState {
  const next = { ...base };

  for (const key of Object.keys(base) as Array<keyof ConvertFormState>) {
    if (values[key] === undefined) {
      continue;
    }

    if (BOOLEAN_FORM_FIELDS.has(key)) {
      (next as Record<string, unknown>)[key] = values[key] !== false;
      continue;
    }

    (next as Record<string, unknown>)[key] = String(values[key]);
  }
