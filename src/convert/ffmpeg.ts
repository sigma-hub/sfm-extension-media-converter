...
import type { ImageConvertOptions, VideoConvertOptions } from './types.js';

function getVideoCodecForFormat(format: string) {
  switch (format) {
...
function appendTrimArgs(args: string[], options: VideoConvertOptions) {
  if (options.trimStart) {
    args.push('-ss', options.trimStart);
  }
}

function appendTrimDurationArgs(args: string[], options: VideoConvertOptions) {
  if (options.trimDuration) {
    args.push('-t', options.trimDuration);
  }
}

function appendGifLoopArgs(args: string[], options: VideoConvertOptions) {
...
function buildGifPaletteFilter(scaleFilter: string, options: VideoConvertOptions) {
...
export function buildVideoArgs(inputPath: string, outputPath: string, options: VideoConvertOptions) {
...
function buildGifArgs(inputPath: string, outputPath: string, options: VideoConvertOptions) {
  const fps = options.videoFramerate && options.videoFramerate !== 'original'
    ? options.videoFramerate
    : '15';

  const scaleFilterBase = buildScaleFilter(
    options.scaleMode,
    options.scaleWidth,
    options.scaleHeight,
    options.scalePercent,
...
export function buildImageArgs(inputPath: string, outputPath: string, options: ImageConvertOptions) {
