...
export function getDirectoryFromPath(filePath: string) {
  if (!filePath) return null;
  return sigma.path.dirname(filePath);
}

export function detectSeparator(filePath: string) {
...
export function normalizeOutputNameKey(fileName: string) {
...
export function buildExistingNamesSet(entries: Array<{ name?: string; path?: string }>) {
...
export async function buildExistingNamesSetForDirectory(
  directoryPath: string,
  fallbackEntries: ClassifiableEntry[],
) {
  try {
...
export function resolveOutputFileName(baseName: string, extension: string, existingNamesSet: Set<string>) {
...
export async function getFileSizeBytesByPath(filePath: string) {
...
export function getEntryInputSizeBytes(entry: ClassifiableEntry) {
...
export async function prepareOutputLocation(
  filePath: string,
  baseName: string,
  extension: string,
  existingNamesSets: Record<string, Set<string>>,
  outputSubfolderSettings: { useSubfolder: boolean; subfolderName: string },
  preparedOutputDirectories: Set<string>,
  fallbackEntries: ClassifiableEntry[],
): Promise<OutputLocation | null> {
  ...
  let outputDirectory = resolveOutputDirectory(
    ...
    separator,
    useSubfolder,
    outputSubfolderSettings.subfolderName,
  );
...
export async function preloadExistingNamesSets(
  files: ClassifiableEntry[],
