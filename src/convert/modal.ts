import type { UIElement } from '@sigma-file-manager/api';
import {
  estimateGifOutputBytes,
  formatByteSizeLabel,
  formatMediaInfoSummary,
...
import type { ConvertFormState, ClassifiableEntry, MediaInfo } from './types.js';
...
function getFramerateOptions(includeOriginal: boolean) {
...
function buildScaleInputs(
  scaleMode: ConvertFormState['videoScaleMode'],
  scaleWidth: string,
  scaleHeight: string,
  scalePercent: string,
  lockAspectRatio: boolean,
  fieldPrefix: 'video' | 'image',
): UIElement[] {
...
function buildGifEstimateAlert(formState: ConvertFormState, mediaInfo: MediaInfo | null): UIElement | null {
  ...
  const estimatedBytes = estimateGifOutputBytes(
    dimensions.width,
    dimensions.height,
    Number.isFinite(fpsValue) ? fpsValue : 15,
    durationSeconds,
  );
...
function buildVideoSection(
  ...
  content.push(...buildScaleInputs(
    formState.videoScaleMode,
    formState.videoScaleWidth,
    formState.videoScaleHeight,
    formState.videoScalePercent,
    formState.videoLockAspectRatio,
    'video',
  ));
...
function buildVideoAdvancedSection(formState: ConvertFormState): UIElement[] {
  const content: UIElement[] = [];
  const isGif = formState.videoFormat === 'gif';
...
function buildImageSection(formState: ConvertFormState, imageFiles: ClassifiableEntry[], hasVideos: boolean): UIElement[] {
  const content: UIElement[] = [];
  ...
  content.push(...buildScaleInputs(
    ...
    formState.imageLockAspectRatio,
    'image',
  ));
...
function shouldRebuildModalContent(
  previousFormState: ConvertFormState,
  nextFormState: ConvertFormState,
  hasVideos: boolean,
  hasImages: boolean,
) {
...
export function createConvertModal(
  videoFiles: ClassifiableEntry[],
  imageFiles: ClassifiableEntry[],
  initialFormState: ConvertFormState,
  mediaInfo: MediaInfo | null,
) {
  const hasVideos = videoFiles.length > 0;
  const hasImages = imageFiles.length > 0;
  ...
  return new Promise<ConvertFormState | null>((resolve) => {
    ...
    modal.onValueChange(async (elementId, value) => {
      if (elementId === 'preset' && value === 'lastUsed') {
        formState = sanitizeFormStateForSelection(
          await loadLastUsedFormState(createDefaultFormState()),
          hasVideos,
          hasImages,
        );
        rebuildContent();
