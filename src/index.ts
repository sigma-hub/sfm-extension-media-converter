...
import {
  ...
  applyModalHint,
  buildImageParamSuffix,
  buildVideoParamSuffix,
  createDefaultFormState,
  getOutputSubfolderSettings,
  loadLastUsedFormState,
  parseImageConvertOptions,
  parseVideoConvertOptions,
  resolveVideoFramerateForFile,
  sanitizeFormStateForSelection,
  saveLastUsedFormState,
  validateConvertFormState,
} from './convert/options.js';
import {
  getFileExtension,
  getFileNameWithoutExtension,
  preloadExistingNamesSets,
  prepareOutputLocation,
} from './convert/output.js';
import { probeMediaInfo } from './convert/probe.js';
import {
  convertImageForReduce,
  convertVideoWithFallback,
  getReduceOutputExtension,
} from './convert/reduce.js';
import {
  classifyFiles,
  ensureFfmpegToolchainAvailable,
  handleInstallActivation,
  ...
  VIDEO_EXTENSIONS,
} from './convert/toolchain.js';
import type { ClassifiableEntry, ConvertModalHint, MediaInfo } from './convert/types.js';

const t = sigma.i18n.extensionT;

async function prepareInitialFormState(
...
function showCancelledNotification(successCount: number, totalSupported: number) {
  sigma.ui.showNotification({
    title: t('extensionTitle'),
...
async function handleConvertCommand(
  initialEntries: ClassifiableEntry[] | null,
  hint?: ConvertModalHint,
) {
  ...
  let sourceMediaInfo: MediaInfo | null = null;
  if (videoFiles.length === 1) {
    sourceMediaInfo = await probeMediaInfo(ffprobePath, videoFiles[0].path);
  }

  const initialFormState = await prepareInitialFormState(
    hint,
    videoFiles.length > 0,
    imageFiles.length > 0,
  );
  const formStateResult = await createConvertModal(
    videoFiles,
    imageFiles,
    initialFormState,
    sourceMediaInfo,
  );
  if (!formStateResult) return;

  const validationError = validateConvertFormState(
    formStateResult,
    videoFiles.length > 0,
    imageFiles.length > 0,
  );
  ...
  const allFiles = [
    ...videoFiles.map((file) => ({ file, type: 'video' as const })),
    ...imageFiles.map((file) => ({ file, type: 'image' as const })),
  ];

  const existingNamesSets = await preloadExistingNamesSets(
    allFiles.map(({ file }) => file),
    entries,
  );
  const preparedOutputDirectories = new Set<string>();
    ...
    processFile: async (context) => {
      ...
      if (type === 'video') {
        const fileMediaInfo = baseVideoOptions.framerateSelect === 'original'
          ...
          : null;
        const videoOptions = resolveVideoFramerateForFile(
          baseVideoOptions,
          fileMediaInfo?.frameRate ?? sourceMediaInfo?.frameRate ?? null,
        );
        ...
        const outputLocation = await prepareOutputLocation(
          file.path,
          baseName + paramSuffix,
          outputFormat,
          existingNamesSets,
          outputSubfolderSettings,
          preparedOutputDirectories,
          entries,
        );
        if (!outputLocation) {
          return 'failed';
  ...
  if (progressResult.cancelled) {
    ...
    showFailureNotification(
      progressResult.failedCount,
      allFiles.length,
      progressResult.successCount,
      progressResult.failedFiles,
      'convert',
    );
  }
}
...
export async function activate(context: ExtensionActivationContext): Promise<void> {
  ...
  await sigma.commands.registerCommand(
    { id: 'convertToGif', title: t('convertToGifCommandTitle'), icon: 'Image' },
    async () => handleConvertCommand(null, { videoFormat: 'gif', preset: 'webGif' }),
  );

  await sigma.commands.registerCommand(
    { id: 'convertToWebp', title: t('convertToWebpCommandTitle'), icon: 'Image' },
    async () => handleConvertCommand(null, { imageFormat: 'webp', preset: 'webOptimized' }),
  );

  await sigma.commands.registerCommand(
    { id: 'convertToMp4Copy', title: t('convertToMp4CopyCommandTitle'), icon: 'Film' },
    async () => handleConvertCommand(null, { preset: 'mp4Copy' }),
  );
