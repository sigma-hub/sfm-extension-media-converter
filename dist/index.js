...
async function convertVideoWithFallback({ ffmpegPath, ffprobePath, file, outputPath, extension, progressCallback, cancellationToken, }) {
...
function buildVideoFormatContent(videoFormat, videoCodecMode, videoFiles, imageFiles) {
    const hasImages = imageFiles.length > 0;
    const content = [];
    if (hasImages) {
...
function buildModalContent(videoFormat, videoCodecMode, videoFiles, imageFiles) {
    const hasVideos = videoFiles.length > 0;
    const hasImages = imageFiles.length > 0;
    const content = [];
...
async function convertSingleFile(ffmpegPath, inputPath, ffmpegArgs, progressCallback, cancellationToken) {
    ...
    const commandTask = await sigma.shell.runWithProgress(ffmpegPath, ffmpegArgs, (payload) => {
...
async function handleConvertCommand(initialEntries) {
        ...
        for (let fileIndex = 0; fileIndex < allFiles.length; fileIndex++) {
            ...
            try {
                ...
                if (type === 'video') {
                    ...
                    const outputFileName = resolveOutputFileName(baseName + paramSuffix, outputFormat, existingNames);
    ...
    const progressResult = await sigma.ui.withProgress({
        ...
        for (let fileIndex = 0; fileIndex < allFiles.length; fileIndex++) {
            ...
            try {
                ...
                const outputFileName = resolveOutputFileName(`${baseName} - reduced`, outputExtension, existingNames);
                const outputPath = `${dirPath}${separator}${outputFileName}`;
                ...
                const resultData = type === 'video'
                    ...
                    : {
                        result: await convertSingleFile(ffmpegPath, file.path, buildReduceImageArgs(file.path, outputPath, extension), progressCallback, token),
