...
export function resolveScaledDimensions(scaleMode, scaleWidth, scaleHeight, scalePercent, sourceWidth, sourceHeight) {
...
function parsePercentFactor(scalePercent) {
    const normalized = String(scalePercent).replace('%', '').trim();
...
function buildScaleFilterFromDimensions(width, height, useLanczos) {
    const flagsSuffix = useLanczos ? ':flags=lanczos' : '';
    return `scale=${width}:${height}${flagsSuffix}`;
}
export function buildScaleFilter(scaleMode, scaleWidth, scaleHeight, scalePercent, lockAspectRatio, useLanczos = false, sourceWidth = null, sourceHeight = null) {
    const flagsSuffix = useLanczos ? ':flags=lanczos' : '';
    if (scaleMode === 'original') {
        return null;
    }
    if (sourceWidth !== null && sourceHeight !== null) {
        ...
        const scaled = resolveScaledDimensions(scaleMode, scaleWidth, scaleHeight, scalePercent, sourceWidth, sourceHeight);
...
export function migrateLegacyScaleFields(stored) {
    const migrated = {};
...
export function validateScaleFields(scaleMode, scaleWidth, scaleHeight, scalePercent) {
